...
class graph_dictionnaire:
    ...
    def ajouter_arc(self, noeud1, noeud2):
        ...
        # ---- DÉCALAGE SIMPLE POUR QUE LA FLECHE N'ALLER PAS AU CENTRE ----
...
def ui_ajouter_jeton_derniere_place():
    if selected_node: graph.ajouter_jeton(selected_node)
def ui_retirer_jeton_derniere_place():
    if selected_node: graph.retirer_jeton(selected_node)
...
def on_right_click(event):
    ...
    node = graph.item_to_node[item]
    if arc_start_node is None: arc_start_node = node
    else:
        if node != arc_start_node: graph.ajouter_arc(arc_start_node, node)
        arc_start_node = None
...
Button(controls, text="DFS", command=lambda: dfs_algo(selected_node)).pack(pady=5)
Button(controls, text="BFS", command=lambda: bfs_algo(selected_node)).pack(pady=5)
...
def color_node(nom_noeud, couleur="yellow"):
    if nom_noeud not in graph.graph: return
...
def dfs_algo(start):
    ...
    def dfs(nom_noeud):
        ...
        data = graph.graph[nom_noeud]
        neighbors = data["transitions"] if data["type"] == "place" else data["sortantes"]
